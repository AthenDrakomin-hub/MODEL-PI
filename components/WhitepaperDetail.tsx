...
// Import ModelPiLogo to fix the missing component error
...
export const WhitepaperDetail = ({ onClose, lang }: { onClose: () => void, lang: string }) => {
  ...
  const getLabel = (zh: string, en: string) => lang === 'zh' ? zh : en;
  ...
  return (
    <div className={`fixed inset-0 z-[250] bg-white text-gray-900 overflow-y-auto animate-in fade-in zoom-in duration-500 custom-scrollbar ${isRTL ? 'text-right' : 'text-left'}`} dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      <button 
        onClick={onClose} 
        className="fixed top-8 right-8 z-[300] p-5 bg-red-600 text-white rounded-full shadow-[0_15px_40px_rgba(232,33,39,0.4)] hover:scale-110 hover:bg-black transition-all active:scale-95 group"
      ...
      <div className="max-w-7xl mx-auto p-6 md:p-16 space-y-24">
        ...
        <div className="relative overflow-hidden bg-neutral-50 rounded-[3.5rem] p-10 md:p-20 border border-gray-100">
           <div className="relative z-10 max-w-3xl">
              ...
              <h2 className="text-4xl md:text-8xl font-black text-gray-900 tracking-tighter uppercase leading-[0.85] mb-8">
                The Next <br/><span className="text-red-600">Civilization</span> Hub.
              </h2>
              <p className="text-xl md:text-2xl text-gray-500 font-medium leading-relaxed">
                {getLabel(
                  "Model Pi 不仅仅是一台手机，它是连接地球、星链与火星通讯协议的终端核心。它是人类向多行星文明跨越的基础设施。",
                  "Model Pi is more than a smartphone. It is the core terminal connecting Earth, Starlink, and Mars communication protocols."
                )}
              </p>
           </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
           <div className="space-y-10">
              <h3 className="text-3xl font-black uppercase tracking-tighter flex items-center gap-4">
                 <BarChart3 className="text-red-600" /> {getLabel("AI 算力压制", "AI Compute Dominance")}
              </h3>
           ...
           <div className="grid grid-cols-2 gap-4">
              ...
              <SpecBox label="Connectivity" val="Starlink V3" detail="10Gbps Uplink" icon={<Satellite size={18}/>} />
        ...
        <div className="space-y-12">
           <h3 className="text-3xl font-black uppercase tracking-tighter">{getLabel("战略支柱", "Strategic Pillars")}</h3>
           <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <StrategyCard 
                icon={<Satellite className="text-red-600" size={32} />} 
                title="Starlink V3 Native"
                desc={getLabel("无需地面基站，直连轨道卫星。实现全球（包括公海与无人区）全天候、零死角通讯。", "Direct-to-orbit connectivity. Full global coverage including high seas and remote deserts.")}
              />
              <StrategyCard 
                icon={<Brain className="text-purple-600" size={32} />} 
                title="Neuralink Sync"
                desc={getLabel("内置非侵入式脑机接口芯片，支持毫秒级意念控屏。为未来人类进化提供标准接口。", "Non-invasive BCI support for thought-based interaction. Standard interface for human evolution.")}
              />
              <StrategyCard 
                icon={<BatteryCharging className="text-yellow-500" size={32} />} 
                title="Hyper-Solar Shield"
                desc={getLabel("机身背板采用第三代高效钙钛矿太阳能涂层。阳光下 1 小时即可补充 40% 电量。", "3rd-gen Perovskite solar coating. 1 hour of sunlight recovers 40% battery capacity.")}
              />
              <StrategyCard 
                icon={<Shield className="text-cyan-500" size={32} />} 
                title="Data Sovereignty"
                desc={getLabel("所有生物识别数据与 T-OS 系统逻辑均在本地 T1 安全芯片运行，完全脱离中心化审查。", "All biometric and OS logic processed locally on the T1 secure chip, free from centralized censorship.")}
              />
           </div>
        </div>
        ...
        <div className="pt-12 border-t border-gray-100 text-center space-y-8">
           ...
           <div className="max-w-2xl mx-auto">
              <h4 className="text-2xl font-black uppercase mb-4">{getLabel("改变一切", "Change Everything")}</h4>
           ...
           >
             {getLabel("了解并返回", "ACKNOWLEDGE & RETURN")}
           </button>
        </div>
