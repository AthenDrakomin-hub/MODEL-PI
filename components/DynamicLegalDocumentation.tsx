// components/DynamicLegalDocumentation.tsx
...
const DynamicLegalDocumentation = () => {
  ...
  useEffect(() => {
    const fetchLegalDocuments = async () => {
      try {
        setLoading(true);
        // 在实际实现中，这里会调用 Worker API
        // const response = await fetch('/api/legal/documents');
        // const data = await response.json();
        
        // 模拟数据 - 在实际部署中，这些将从 Worker API 获取
...
产品符合相关地区法规要求。`,
        ...
        const docMap: Record<string, LegalDocument> = {};
  ...
  const legalSections = {
    privacy: { title: '隐私政策', key: 'privacy' },
    terms: { title: '服务条款', key: 'terms' },
    warranty: { title: '保修政策', key: 'warranty' },
    refund: { title: '退货政策', key: 'refund' },
    legal: { title: '法律声明', key: 'legal' },
    compliance: { title: '合规文件', key: 'compliance' }
  };
    ...
    return (
      <div className="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-lg">
        <div className="text-center py-12">
          ...
          <p className="text-gray-600">加载法律文件中...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-lg">
        <div className="text-center py-12">
          ...
          <p className="text-red-600">加载法律文件时出错</p>
  ...
  const currentDoc = documents[activeSection as keyof typeof documents];
      ...
      {currentDoc ? (
        <div className="prose prose-lg max-w-none bg-gray-50 p-6 rounded-lg">
          <div className="mb-4 text-sm text-gray-500">
            版本: {currentDoc.version} | 更新时间: {new Date(currentDoc.lastUpdated).toLocaleDateString()}
          </div>
          <div 
            className="legal-content"
            dangerouslySetInnerHTML={{ 
              __html: currentDoc.content
                .replace(/\n# /g, '<h1 class="text-2xl font-bold mt-8 mb-4 text-gray-800">')
                .replace(/\n## /g, '</h1><h2 class="text-xl font-bold mt-6 mb-3 text-gray-700">')
                .replace(/\n### /g, '</h2><h3 class="text-lg font-bold mt-4 mb-2 text-gray-600">')
                .replace(/\n- /g, '<br/>• ')
                .replace(/<br\/>• /, '• ')
                .replace(/\n\n/g, '</div><div>')
                .replace(/\n/g, '<br/>')
...
export default DynamicLegalDocumentation;