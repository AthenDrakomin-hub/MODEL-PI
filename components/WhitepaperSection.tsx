      ...
      <div className="max-w-7xl mx-auto relative z-10">
        <div className="mb-10 md:mb-20 flex flex-col md:flex-row justify-between items-center md:items-end gap-6 md:gap-8 text-center md:text-left">
          <div className="space-y-3 md:space-y-4">
            ...
            <h2 id="whitepaper-title" className="text-2xl sm:text-3xl md:text-6xl font-black uppercase tracking-tighter leading-tight sm:leading-none">{t.whitepaper.title}</h2>
          </div>
          <button 
            onClick={onOpenFull}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 md:gap-24 items-start">
          <div className="lg:sticky lg:top-40 space-y-6 md:space-y-12">
            <div 
            ...
            >
              ...
              <div className="perspective-1000 rotate-slow flex justify-center py-4 md:py-10 transition-transform group-hover:scale-105">
                <img 
                  src="https://aka.doubaocdn.com/s/KqXq1vl32h" 
                  alt="Tesla Model Pi 3D Blueprint" 
                  className="w-full max-w-[160px] xs:max-w-[200px] sm:max-w-sm drop-shadow-[0_0_80px_rgba(232,33,39,0.1)]" 
                />
              </div>
            </div>
            
            {/* Tabs - Better layout for mobile */}
            <div className="flex justify-between space-x-1.5 sm:space-x-4" role="tablist">
              {t.whitepaper.tabs.map((tab: string, idx: number) => (
                <button 
                  ...
                  className={`flex-1 py-3 sm:py-4 px-1 rounded-xl sm:rounded-2xl text-[7px] xs:text-[9px] sm:text-[10px] font-black uppercase tracking-widest border transition-all active:scale-95 ${activeTab === idx ? 'bg-red-600 border-red-600 text-white' : 'bg-white/5 border-white/10 text-gray-500 hover:text-white'}`}
                >
                  {tab}
                </button>
              ))}
          ...
          <div className="space-y-6 md:space-y-10">
            {activeTab === 0 && (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-6" role="tabpanel">
                {t.whitepaper.specs.map((spec: any, i: number) => {
                  const Icon = ICONS[spec.icon] || Info;
                  return (
                    <article key={i} className="bg-white/[0.03] border border-white/5 p-5 md:p-8 rounded-xl md:rounded-3xl hover:bg-white/[0.05] transition-all group">
                      <div className="flex items-center space-x-2 mb-2 md:mb-3 text-red-500">
                        <Icon size={14} aria-hidden="true" className="md:size-4" />
                        <h3 className="text-[8px] md:text-[10px] font-black uppercase text-gray-500 tracking-wider">{spec.label}</h3>
                      </div>
                      <div className="text-base md:text-xl font-black uppercase mb-0.5 md:mb-1 leading-tight">{spec.value}</div>
                      <p className="text-[9px] md:text-xs text-gray-500 font-mono tracking-tight">{spec.detail}</p>
                    </article>
                  );
                })}
              </div>
            ...
            {activeTab === 1 && (
              <div className="space-y-4 md:space-y-6" role="tabpanel">
                {t.whitepaper.sellingPoints.map((point: any, i: number) => (
                  <article key={i} className="group flex flex-col items-start gap-3 sm:gap-8 bg-white/[0.02] border border-white/5 p-6 md:p-10 rounded-2xl sm:rounded-[2.5rem] hover:bg-white/[0.04]">
                    <div className="space-y-2 md:space-y-4">
                      <div className="flex flex-wrap items-center gap-2 sm:gap-3">
                        <h3 className="text-lg md:text-2xl font-black uppercase tracking-tighter">{point.title}</h3>
                        <span className="px-2 py-0.5 bg-red-600/10 border border-red-600/30 text-[7px] md:text-[9px] font-black uppercase text-red-500 rounded">{point.meta}</span>
                      </div>
                      <p className="text-gray-400 leading-relaxed text-[11px] md:text-sm">{point.desc}</p>
                    </div>
                  </article>
                ))}
              </div>
                ...
                {t.whitepaper.future.map((fut: any, i: number) => {
                  ...
                  return (
                    <article key={i} className="p-8 md:p-12 rounded-2xl md:rounded-[3.5rem] glass-effect border-red-600/10 text-center space-y-4 md:space-y-8">
                      {Icon && <Icon size={32} className="md:size-12 mx-auto text-red-600 mb-1" aria-hidden="true" />}
