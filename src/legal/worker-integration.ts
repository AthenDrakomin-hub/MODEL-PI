// src/legal/worker-integration.ts
// Tesla Model π 法律文件 Worker 集成
...
// 扩展 LegalService 以集成其他服务
export class IntegratedLegalService extends LegalService {
  constructor(env: Env) {
    super(env);
  }
...
产品符合相关地区法规要求。`,
    ...
    for (const doc of defaultDocuments) {
      // 检查是否已存在该类型的文档
      const existing = await this.getLatestDocumentByType(doc.type);
      if (!existing) {
        await this.createOrUpdateLegalDocument(doc);
      }
    }
  }
}
...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    const integratedLegalService = new IntegratedLegalService(env);
    const url = new URL(request.url);
    const path = url.pathname;

    try {
      // 初始化默认文档（仅在需要时）
      if (path === '/api/legal/init' && request.method === 'POST') {
        await integratedLegalService.initializeDefaultDocuments();
        return new Response(JSON.stringify({
          success: true,
          message: 'Legal documents initialized'
        }), {
      ...
      // 获取特定法律文档
      if (path.startsWith('/api/legal/document/') && request.method === 'GET') {
        ...
        const document = await integratedLegalService.getLegalDocument(documentId);
        if (!document) {
          return new Response(JSON.stringify({
            success: false,
      ...
      // 获取所有公开法律文档
      if (path === '/api/legal/documents' && request.method === 'GET') {
        const documents = await integratedLegalService.getPublicLegalDocuments();
      ...
      // 获取合规证书
      if (path.startsWith('/api/legal/certificate/') && request.method === 'GET') {
        ...
        const certificate = await integratedLegalService.getComplianceCertificate(certificateId);
        if (!certificate) {
          return new Response(JSON.stringify({
            success: false,
      ...
      // 获取所有合规证书
      if (path === '/api/legal/certificates' && request.method === 'GET') {
        const certificates = await integratedLegalService.getAllComplianceCertificates();
      ...
      // 上传合规证书
      if (path === '/api/legal/upload/certificate' && request.method === 'POST') {
        const formData = await request.formData();
        const file = formData.get('file') as File;
        const certificateType = formData.get('certificateType') as 'FCC' | 'CE' | 'UKCA' | 'OTHER';
        const certificateId = formData.get('certificateId') as string;

        if (!file || !certificateType || !certificateId) {
          return new Response(JSON.stringify({
            success: false,
            message: 'File, certificateType, and certificateId are required'
          }), {
            status: 400,
            headers: { 'Content-Type': 'application/json' }
          });
        ...
        const certificate = await integratedLegalService.uploadComplianceCertificate(
          arrayBuffer,
          file.name,
          certificateType,
          certificateId
